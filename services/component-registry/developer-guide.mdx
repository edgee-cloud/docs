...
Commands:
  ...
  components new    Create a new component with sample code.
  components init   Initialize a new component manifest in the current folder.
  components build  Compile the component in the current folder to Wasm.
  components test   Run the component in the current folder with sample events.
  components check  Check if the local .wasm file is valid.
```
