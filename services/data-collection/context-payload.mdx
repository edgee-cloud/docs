...
The payload is defined in a `<script>` tag with `type="application/json"` and a specific ID, making it easily identifiable and parseable by Edgee. 
...
Use the `destinations` object to specify which analytics integrations **should not** receive the data. If you do not specify any destinations, 
...
To exclude Google Analytics from receiving the data, you can set the `Google Analytics` field to `false`:
