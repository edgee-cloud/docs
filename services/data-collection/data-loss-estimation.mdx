...
description: Learn how we estimate data loss on your website due to user consent requirements, adblockers, and client-side performance issues.
---

Edgee provides a [tool](https://www.edgee.cloud/data-collection) to help you understand data collection 
...
## Overview of the Data Loss Estimation Tool

Our tool functions by taking a URL input from the user and launching a crawler that navigates the specified webpage. 
The crawler simulates user interactions and observes all data collection requests 
initiated by the page within the browser environment.

### How It Works

1. **Input URL**: Users input the URL of the webpage they wish to analyze.
2. **Crawling**: Our tool crawls the webpage, simulating standard user interactions to trigger data collection requests.
3. **Request Analysis**: For each data request, the tool assesses potential obstacles that might prevent the data from being successfully collected.
...
- **Average Data Loss**: Calculates an average data loss based on the types and frequencies of obstacles encountered on the webpage.

## Estimating Data Loss for Loyal Users

<img className="block" src="/images/loyal-user-toggle.png" alt="Loyal users" width={200} />

Our tool also considers the behavior of loyal users (those with more than four sessions per day) and 
estimates data loss specifically for this user segment. The rationale is that loyal users, due to 
their frequent interactions with the site, are more likely to encounter and potentially activate 
the obstacles identified, leading to higher risks of data loss.

## Utilizing the Tool

- **Access**: The Data Loss Estimation Tool is available through the Edgee platform. Users can access it by logging into their dashboard.
- **Report**: After analysis, the tool provides a comprehensive report detailing each found obstacle, the potential data loss, 
