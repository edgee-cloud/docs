...
When you fire a `user` event using Edgee’s JavaScript SDK or Data Layer, Edgee securely processes and stores the user data at the edge. 
...
But some components support the `user` event natively. It is the case for Amplitude or Segment for example. 
