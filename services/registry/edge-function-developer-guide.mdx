---
title: Edge Function Developer Guide
description: Step-by-step guide for edge function component developers
icon: lightning-bolt
---
...
Edge functions are WebAssembly-based HTTP handlers that generate custom responses for specific paths.
They're triggered by path matching and run in a sandboxed environment with limited capabilities.
...
$ edgee components new --name my-edge-function --language rust --type edge-function
 INFO Downloading sample code for Rust edge-function...
 INFO Extracting code...
 INFO Downloading WIT files...
 INFO New project my-edge-function is ready! Check README for dependencies.
...
### Implementing your edge function component
...
- **Export** `wasi:http/incoming-handler@0.2.0` - handle incoming HTTP requests
- **Import** `wasi:http/outgoing-handler@0.2.0` - make outgoing HTTP requests when needed

Your edge function receives HTTP requests and must return custom responses. The component settings configured in your manifest are automatically passed to your function via the `x-edgee-component-settings` HTTP header as JSON.

### How edge functions are triggered

Edge functions are triggered by path matching using these special settings:
- `edgee_path`: Exact path match (e.g., `/api/auth`)
- `edgee_path_prefix`: Path prefix match (e.g., `/api/` matches `/api/users`, `/api/posts`, etc.)

When a request matches your configured path, Edgee will:
1. Add your component settings as the `x-edgee-component-settings` header
2. Invoke your edge function with the HTTP request
...
<Note>
  **Important Limitations**: Edge functions run in a sandboxed WebAssembly environment with restricted capabilities:
  - No persistent storage (databases, files, or stateful data)
  - Cannot forward requests to backend services
  - No direct access to Edgee's data layer or analytics
  - Limited to HTTP-based external communication only
  - Must return a response for every request
</Note>
...
For example, a simple API edge function might expect `api_key` and `allowed_origins` settings,
while a content serving edge function might expect `content_type` and `template_url` settings.

Additionally, you'll need to configure path matching to specify when your edge function should be triggered:
- `edgee_path`: For exact path matches
- `edgee_path_prefix`: For path prefix matches
...
# Path matching settings (choose one)
[component.settings.edgee_path]
title = "Exact Path Match"
type = "string"
required = false
description = "Exact path that triggers this edge function (e.g., '/api/auth')"

[component.settings.edgee_path_prefix]
title = "Path Prefix Match"
type = "string"
required = false
description = "Path prefix that triggers this edge function (e.g., '/api/' matches all paths starting with /api/)"
```
...
You can parse this header to access your component's configuration:

```rust
// Example in Rust
let settings_header = request.headers().get("x-edgee-component-settings");
if let Some(settings) = settings_header {
    let settings_json: serde_json::Value = serde_json::from_str(settings.to_str().unwrap()).unwrap();
    let jwt_secret = settings_json["jwt_secret"].as_str().unwrap();
}
```
...
The test command starts a local HTTP server running your edge function. You'll need to use curl or another HTTP client to send requests to the server URL to test your component's behavior.
...
Test HTTP requests represent typical HTTP requests that your edge function will process.

For example, a sample HTTP request might look like:
...
- Make HTTP requests to external services for data
- Generate and return a custom HTTP response

<Note>
  Test HTTP requests are configurable. You can provide custom test requests that better suit your component's needs
...
<Note>
  ...
  affecting production systems or external APIs during testing.
</Note>
...
1. By default, components are created as inactive to avoid unintended errors - don't forget to mark it as active to start processing requests.
...
4. Configure the path matching by setting either:
...
When a request matches your configured path, Edgee will:
1. Add your component settings as the `x-edgee-component-settings` HTTP header
2. Invoke your edge function with the HTTP request
3. Return your function's response to the client
...
<Note>
  **Performance Considerations**: Edge functions run on every matching request in a WebAssembly sandbox.
  ...
  Edge functions must handle all requests for their configured paths - they cannot forward requests to backend services.
</Note>