...
| `compression_rate` | `f64` | The compression rate to use for this request. If `enable_compression` is `true`, this value will be used to compress the request. The value should be between 0.0 and 1.0. The default value is 0.75. |
...
```rust
let response = client.send("gpt-4o", "Analyze this long document with lots of context...").await?;
