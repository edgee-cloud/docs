...
3.  Set both the **Cookie Domain** and **Frontend Hostname** fields(in **Console** > **Project Settings > Compute**) to your custom domain (e.g., `mywebsite.com`) in  
4.  Secure the connection by adding [required secret headers](services/security/access-control#required-header) in Edgee. This ensures only authorized requests from Edgee are accepted by your CDN and forwarded to your cloud/website.
