...
1.  **Configure Your CDN**  
    Set your CDN to forward all incoming traffic to your Edgee endpoint (e.g., `yourproject.edgee.app`).
2.  **Set Up Routing in Edgee**  
    Configure Edgee to [route traffic to your domain](/getting-started/routing).
3.  **Update Project Settings**  
    In the **Console**, go to **Project Settings > Compute**, and ensure both the **Cookie Domain** and **Frontend Hostname** fields are set to your custom domain (e.g., `mywebsite.com`).
4.  **Add Security Headers**  
    Add the [required secret headers](services/security/access-control#required-header) to your CDN configuration to ensure that only authorized requests from Edgee are accepted.
   
<Note>
Note: Edgee will still track client-side events—even if your CDN caches full HTML pages—so you don’t lose visibility into user interactions.
</Note>
