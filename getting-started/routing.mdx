...
These rules are applied using an "AND" logic, meaning they all need to be true in order to match an incoming request.
...
<Note>
    ...
    On the other hand, if you configure incompatible rules, no traffic will ever match this route.
    For example, if you select "Europe" as continent and "US-Central" as region, this route will never be used.
</Note>
