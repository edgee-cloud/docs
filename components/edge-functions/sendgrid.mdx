---
title: SendGrid
description: Send transactional emails using SendGrid's powerful email delivery platform, **powered by edge functions**.
---

<img src="/images/banners/sendgrid.png" className="rounded-lg" noZoom />

import EdgeeSdk from '/snippets/edgee-sdk.mdx';

<EdgeeSdk />

Find it on GitHub: [<Icon icon="github" iconType="solid" /> /edgee-cloud/sendgrid-component](https://github.com/edgee-cloud/sendgrid-component)

A WebAssembly component that provides a simple way to send emails via SendGrid directly from the edge. This component can be mapped to a specific endpoint (like `/contact`) and supports both static text and dynamic email templates.

## Features

- **SendGrid Integration**: Direct integration with SendGrid's email API
- **Template Support**: Use SendGrid's dynamic email templates
- **Configurable Endpoint**: Set custom paths for your email sending endpoint
- **Edge Performance**: Fast email processing directly at the edge
- **Static and Dynamic Content**: Support for both static text and templated emails
- **WebAssembly Performance**: Secure, fast execution in a sandboxed environment

## Getting Started

**To integrate SendGrid with your project:**

1. Create a SendGrid account and generate an API key
2. Verify your sender email address in SendGrid
3. Open the Edgee console and navigate to your project's Components
4. Select "Add a component" and choose `sendgrid` from the list of available edge functions
5. Configure the following settings:
   - **API Key**: Your SendGrid API key
   - **From Email**: Your verified sender email address
   - **Subject**: Default email subject line (optional)
   - **Template ID**: SendGrid template ID for dynamic emails (optional)
   - **Path Configuration**: Set the email sending endpoint path (e.g., `/contact`)
6. Click Save to activate the email sending endpoint

## Configuration Options

### Required Settings
- **API Key**: Your SendGrid API key
- **From Email**: Verified sender email address
- **Endpoint Path**: The path for sending emails (e.g., `/contact`)

### Optional Settings
- **Subject**: Default email subject line
- **Template ID**: SendGrid template ID for dynamic emails

### Setup Requirements
- Active SendGrid account
- Verified sender email address in SendGrid
- SendGrid API key with email sending permissions

## Usage

Once configured, you can send emails by making POST requests to your configured endpoint:

```javascript
// Send email with dynamic data
await fetch('/contact', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json'
  },
  body: JSON.stringify({
    "message": "Hello world!",
    "email": "test@example.com"
  })
});
```

### Request Format
```json
{
  "message": "Your email message content",
  "email": "recipient@example.com"
}
```

### Template Support
If you configure a `template_id`, the component will use SendGrid's dynamic template system with the provided data.


## Email Templates

### Dynamic Templates
SendGrid's dynamic templates allow you to create reusable email designs:

```html
<!-- In your SendGrid template -->
<h1>Hello {{user_name}}!</h1>
<p>Your order {{order_number}} is confirmed.</p>
<p>Total: {{order_total}}</p>

{{#if items}}
<ul>
  {{#each items}}
  <li>{{name}} - {{price}}</li>
  {{/each}}
</ul>
{{/if}}
```

### Template Data
```json
{
  "template_id": "d-123456789",
  "dynamic_template_data": {
    "user_name": "John Doe",
    "order_number": "ORD-001",
    "order_total": "$99.99",
    "items": [
      {"name": "Product A", "price": "$49.99"},
      {"name": "Product B", "price": "$49.99"}
    ]
  }
}
```

## Use Cases

1. **Contact Forms**: Process contact form submissions
2. **Transactional Emails**: Order confirmations and notifications
3. **User Onboarding**: Welcome emails and account setup
4. **Support Tickets**: Automated support email responses
5. **Marketing Communications**: Newsletter and promotional emails
6. **System Notifications**: Alert emails for system events

## Example Integrations

### Order Confirmation Email
```javascript
fetch('/api/email/send', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
  },
  body: JSON.stringify({
    to: 'customer@example.com',
    template_id: 'd-order-confirmation',
    dynamic_template_data: {
      customer_name: 'John Doe',
      order_number: 'ORD-001',
      order_total: '$99.99',
      items: [
        {name: 'Product A', quantity: 1, price: '$99.99'}
      ],
      shipping_address: '123 Main St, City, State 12345'
    }
  })
})
.then(response => response.json())
.then(data => {
  console.log('Email sent:', data);
});
```

### Password Reset Email
```javascript
fetch('/api/email/send', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
  },
  body: JSON.stringify({
    to: 'user@example.com',
    template_id: 'd-password-reset',
    dynamic_template_data: {
      user_name: 'John Doe',
      reset_link: 'https://yourapp.com/reset-password?token=abc123',
      expires_at: '2024-01-15 18:00:00'
    }
  })
});
```

### Newsletter Campaign
```javascript
fetch('/api/email/bulk', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
  },
  body: JSON.stringify({
    template_id: 'd-newsletter',
    recipients: subscribers.map(subscriber => ({
      to: subscriber.email,
      dynamic_template_data: {
        name: subscriber.name,
        preferences: subscriber.preferences,
        unsubscribe_link: `https://yourapp.com/unsubscribe?token=${subscriber.token}`
      }
    }))
  })
});
```

## Advanced Features

### Custom Headers
```json
{
  "to": "recipient@example.com",
  "subject": "Custom email",
  "html": "<p>Email content</p>",
  "headers": {
    "X-Custom-Header": "value",
    "X-Priority": "1"
  }
}
```

### Scheduled Sending
```json
{
  "to": "recipient@example.com",
  "subject": "Scheduled email",
  "html": "<p>This email will be sent later</p>",
  "send_at": 1640995200
}
```

### Categories and Tags
```json
{
  "to": "recipient@example.com",
  "subject": "Tagged email",
  "html": "<p>Email content</p>",
  "categories": ["marketing", "newsletter"],
  "custom_args": {
    "campaign_id": "camp_123",
    "user_id": "user_456"
  }
}
```

## Webhook Integration

Handle SendGrid webhook events to track email activity:

```javascript
// Configure webhook endpoint
const webhookEvents = [
  'delivered',
  'opened',
  'clicked',
  'bounced',
  'dropped',
  'unsubscribed'
];
```

Common webhook events:
- `delivered`: Email successfully delivered
- `opened`: Email opened by recipient
- `clicked`: Link clicked in email
- `bounced`: Email bounced
- `dropped`: Email dropped by SendGrid
- `unsubscribed`: Recipient unsubscribed

## Error Handling

The edge function provides comprehensive error handling:

- **400 Bad Request**: Invalid request format or missing required fields
- **401 Unauthorized**: Invalid API key or authentication failure
- **403 Forbidden**: Insufficient permissions or suspended account
- **413 Payload Too Large**: Email content or attachments too large
- **429 Too Many Requests**: Rate limit exceeded
- **500 Internal Server Error**: SendGrid API error

## Security Features

- **API Key Encryption**: Your SendGrid API key is stored securely
- **Rate Limiting**: Automatic rate limiting to prevent abuse
- **Input Validation**: All inputs are validated before processing
- **SPF/DKIM**: Email authentication for improved deliverability

## Best Practices

1. **Authenticate Your Domain**: Set up SPF, DKIM, and DMARC records
2. **Use Templates**: Create reusable templates for consistent branding
3. **Monitor Reputation**: Keep track of bounce rates and spam complaints
4. **Segment Your Lists**: Send targeted emails to specific audiences
5. **Test Before Sending**: Always test emails before bulk sending
6. **Handle Unsubscribes**: Respect unsubscribe requests promptly

<Note>
  Maintain good sending practices to ensure high deliverability rates. 
  Monitor your sender reputation and bounce rates regularly.
</Note>

For more details on SendGrid's API and features, refer to the 
[official SendGrid documentation](https://docs.sendgrid.com/api-reference).