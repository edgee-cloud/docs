...
3. Select "Add a component" and choose `stripe-billing-portal` from the list of available edge functions
...
### Payment Methods
- Add new credit cards, bank accounts, or other payment methods
- Update existing payment methods
- Set default payment method for subscriptions
- Remove outdated payment methods

### Subscription Management
- View current subscriptions and their status
- Upgrade or downgrade subscription plans
- Cancel subscriptions with customizable retention flows
- Reactivate cancelled subscriptions

### Invoice Management
- View invoice history and payment status
- Download invoices as PDF
- Update billing information and tax details
- Set up automatic payments

### Usage-Based Billing
- View current usage and upcoming charges
- Set usage alerts and spending limits
- Review usage history and trends
...
```javascript
// Create customer and redirect to portal
fetch('/api/billing/portal/url', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
  },
...
Handle Stripe webhook events to keep your application synchronized:
...
- `payment_method.attached`: New payment methods added
...
```json
{
  "locale": "en",
  "configuration": {
    "default_return_url": "https://yourapp.com/dashboard"
  }
}
```
...
- **401 Unauthorized**: Invalid Stripe API key
...
- **API Key Encryption**: Your Stripe keys are stored securely
- **Webhook Verification**: Automatic verification of webhook signatures
- **Customer Validation**: Ensures customers can only access their own data
- **HTTPS Required**: All communications are encrypted

<Note>
  The Stripe Billing Portal is PCI DSS compliant and handles all sensitive payment information securely. 
  Customers never enter payment details directly into your application.
</Note>

For more details on Stripe's Billing Portal and API, refer to the 
[official Stripe documentation](https://stripe.com/docs/billing/subscriptions/customer-portal).