...
When a user lands on your site from a LinkedIn ad with the `li_fat_id` URL parameter (e.g., `?li_fat_id=xxx`), Edgee:

1. **Captures the parameter** from the URL
2. **Sets a cookie** (`_li_fat_id`) with a 90-day expiry
3. **Maintains attribution** across all subsequent page views and conversions

This ensures LinkedIn can properly attribute conversions even when users navigate through your site or return later.

### Cookie Format

- **Name**: `_li_fat_id`
- **Format**: `li.1.{timestamp}.{li_fat_id}`
- **Expiry**: 90 days
