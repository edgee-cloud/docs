...
Implementing the LinkedIn Conversion API via Edgee is simple and straightforward. This document will guide you on:
...
Once your LinkedIn CAPI tag is configured, it's time to implement it on your property. We have implemented LinkedIn CAPI to work alongside your InsightTag based on Microsoft's best practices ([here](https://learn.microsoft.com/en-us/linkedin/marketing/conversions/deduplication?view=li-lms-2025-07)) to ensure conversion deduplication.
...
Let's look at tracking a form submission where you have an email address:

1. You will need to track the User event with their email (please note, we will SHA256 when sending to LinkedIn)
...
```
   ...
   (Please note: replace [YOUR_CONVERSION_PIXEL_ID] with your LinkedIn CAPI ID.)

   


```
<script>const trackLinkedInFormSubmission = () => {   edgee.user({      "properties": { "email": "me@example.com"}      });     edgee.track(           {name:"urn:lla:llaPartnerConversion:20465700"}           );};  if (!window.edgee) {
    window.addEventListener('edgee:loaded', trackLinkedInFormSubmission);
  } else {     trackLinkedFormSubmission();}</script>

```

3. **Track standard landing page conversion without having the user’s email:**
   ...
   (Please note: the highlighted part in red is where you will insert your LinkedIn CAPI ID.)

```
<script>
  const trackLinkedView = () => {
    edgee.track({ name: "urn:lla:llaPartnerConversion:20465684"});
  };

  if (!window.edgee) {
    window.addEventListener('edgee:loaded', trackLinkedView);
  } else {
    trackLinkedVideoView();}</script>
```
...
```javascript
edgee.track( {
  name: "urn:lla:llaPartnerConversion:20465700",    
...
1. How to create LinkedIn CAPI.  
2. How to enable it with Edgee.  
3. How to track different types of conversions.  
4. How to prevent conversion duplication if you use InsightTags.

If you encountered any problems during these steps, please feel free to contact us.
